...
const ProductDetailPage = async ({ params }: PageProps) => {
  ...
  const product : detailedProductType = await getDetailedProduct(id);
  const reviews : ReviewType[] = await getReviews(product.id);
  const products : productWithStatsType[] = await getFourProductsByCategory(product.categories_products[0].categories.id, product.id);
  ...
  return (
    <div>
      ...
      <div className="mt-20 mb-20">
          <DescriptionReviewsSwitch reviews={reviews} reviewCount={product._count.reviews} longDescription={product.long_description} productId={product.id}/>
      </div>
