...
export async function POST(request) {
  ...
  if (accountId) {
    ...
    sessionParams.payment_intent_data = {
      application_fee_amount: "here the variable after mathing out 10% of the amount in smallest unit",
      transfer_data: {
  ...
  if(session /*still need to check what sripe returns into session if error on create checkout session*/){
    return NextResponse.json({clientSecret: session.client_secret});
  }else{
    return NextResponse.json({error: "stripe checkout error: couldnt create the session"});
  }

  }else{
    return NextResponse.json({error: "stripe checkout error: no account id provided"});
  }

  }else{
    return NextResponse.json({error: "stripe checkout error: no products provided"});
  }
}
