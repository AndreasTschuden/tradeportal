...
const ShippingForm: React.FC<Props> = ({ onSubmit }) => {
  ...
  const handleFormSubmit = (data: ShippingForm) => {
    if (onSubmit) {
      onSubmit(data);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4" noValidate>
      ...
      <div className="flex flex-col">
        ...
        />
        {errors.street && (
          <span className="text-red-500 text-sm mt-1">{errors.street.message}</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col">
          ...
          />
          {errors.zip && (
            <span className="text-red-500 text-sm mt-1">{errors.zip.message}</span>
          )}
        </div>
